...
def tab_cloud(project_id: str, config: dict):
    st.caption(
        "Публикация без Node.js — на случай если это окружение без установленного Node "
        "(например, Streamlit Cloud). Использует Playwright headless вместо Puppeteer. "
        "publish.js при этом никуда не делся — это просто ещё один способ публикации."
    )
...
def show_main(project_id: str):
    ...
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(
        ["Новый пост", "Актуализация", "Запуск", "Облако", "Отчёты и логи", "Настройки"]
    )
    with tab1:
