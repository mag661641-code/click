...
function clientApp() {
  ...
  const state = {
    ...
    // ── Соцсети (Telegram/VK/OK/Дзен/Макс) — ПОЛНОСТЬЮ ОТДЕЛЬНО от Яндекс.Бизнеса. ──
    // У Яндекса пост размножается по выбранным странам/городам (см. currentDraft выше).
    // Соцсети публикуются как ОДИН пост на бренд — без городов, свой текст и картинки,
    // не связаны с draft.body/draft.localImages, чтобы не путать эти два разных сценария.
    socialPost: {
      text: '',
      imageUrl: '',
      localImages: [],                // Загруженные локальные файлы [{fileName, url, originalName, path}]
      platforms: { telegram: false, vk: false, ok: false, dzen: false, max: false },
      dzenPubType: 'post',            // article | post
      publishMode: 'now',             // now | schedule
      scheduledAt: '',                // ISO-строка из datetime-local
    },
    collapsedCountries: {},
    ...
    social: {                            // токены соцсетей текущего серверного проекта (СМУ/ИМП/МПЕ)
      telegram: { botToken: '', chatId: '' },
      vk: { token: '', groupId: '', phone: '', groupUrl: '' },
      ok: { accessToken: '', publicKey: '', secretKey: '', groupId: '' },
      dzen: { login: '', password: '' },
      max: { phone: '' },  // у Макса нет пароля — вход по SMS-коду, только вручную
    },
    socialTestStatus: {},                // { telegram: 'ok'|'err'|'checking', ... }
    socialLoginStatus: {},               // { vk: 'ok'|'err', ok: 'ok'|'err', dzen: ..., max: ... } — результат последнего ручного входа через браузер
  };
  ...
  // ─── СОЦСЕТИ: токены хранятся на сервере отдельно от projects-config.json ──
  // (users-data/{projectId}/social-config.json), т.к. привязаны к серверному
  // проекту (СМУ/ИМП/МПЕ), а не к клиентскому "бренду" внутри state.projects.
  async function loadSocialConfig() {
    try {
      const r = await api('/api/social/config');
      if (r && r.config) state.social = r.config;
    } catch (e) {
      console.error('loadSocialConfig error:', e);
    }
  }
  async function saveSocialConfig() {
    try {
      await api('/api/social/config', { method: 'POST', body: { config: state.social } });
    } catch (e) {
      console.error('saveSocialConfig error:', e);
    }
  }
  async function testSocialConnection(platform) {
    state.socialTestStatus[platform] = { status: 'checking' };
    renderContent();
    try {
      // Проверка идёт на сервере по сохранённому файлу конфига — сохраняем
      // текущие значения полей ПЕРЕД тестом, иначе только что введённые
      // (но не сохранённые кнопкой «Сохранить») значения не будут учтены.
      await saveSocialConfig();
      const r = await api('/api/social/test', { method: 'POST', body: { platform } });
      if (r && r.ok) {
        state.socialTestStatus[platform] = { status: 'ok', mode: r.mode || null };
        toast('Подключение проверено', 'ok');
      } else {
        state.socialTestStatus[platform] = { status: 'err' };
        toast(r && r.error ? r.error : 'Ошибка проверки', 'err');
      }
    } catch (e) {
      state.socialTestStatus[platform] = { status: 'err' };
  ...
  function updateSocialField(platform, field, value) {
    if (!state.social[platform]) state.social[platform] = {};
    state.social[platform][field] = value;
  }
  function saveSocialSettings() {
    saveSocialConfig();
    toast('Токены сохранены', 'ok');
  }
  ...
  async function refreshServerState() {
    try {
      ...
      if (justFinished) {
        ...
        // Для входа/проверки входа в соцсети через браузер — запоминаем результат,
        // чтобы показать ПОСТОЯННЫЙ значок ✅/❌ рядом с кнопками в Настройках
        // (тост исчезает через пару секунд и легко пропустить). Кнопка «Проверить
        // статус входа» даёт честный ответ на вопрос «я сейчас реально авторизован?»
        // в любой момент — не зависит от того, что показывалось сразу после --login.
        if (typeof actionName === 'string' && (actionName.startsWith('login-') || actionName.startsWith('check-'))) {
          const platform = actionName.slice(actionName.indexOf('-') + 1);
          state.socialLoginStatus[platform] = s.process.status === 'done' ? 'ok' : 'err';
        }
      }

      if (state.activeTab === 'run' || state.activeTab === 'report' || state.activeTab === 'actualize' || state.activeTab === 'settings') {
  ...
  const LOGIN_ACTION_NAMES = {
    login: 'Вход в Яндекс',
    'login-vk': 'Вход в ВКонтакте',
    'login-ok': 'Вход в Одноклассники',
    'login-dzen': 'Вход в Дзен',
    'login-max': 'Вход в Макс',
  };
  const CHECK_ACTION_NAMES = {
    'check-vk': 'Проверка входа в ВКонтакте',
    'check-ok': 'Проверка входа в Одноклассники',
    'check-dzen': 'Проверка входа в Дзен',
    'check-max': 'Проверка входа в Макс',
  };
  function notifyProcessFinished(action, status) {
    const isOk = status === 'done';
    const isLoginAction = !!LOGIN_ACTION_NAMES[action];
    const isCheckAction = !!CHECK_ACTION_NAMES[action];
    const actionName = action === 'publish' ? 'Публикация'
      : action === 'actualize' ? 'Актуализация'
      : LOGIN_ACTION_NAMES[action] || CHECK_ACTION_NAMES[action] || 'Процесс';
    const title = isOk ? `✅ ${actionName} завершена` : `❌ ${actionName} завершилась с ошибкой`;
    const body = isCheckAction
      ? (isOk ? 'Вы авторизованы, сессия активна.' : 'Сессия не найдена — нужно войти заново.')
      : isOk
        ? (isLoginAction ? 'Аккаунт успешно подключен, сессия сохранена.' : 'Откройте отчёт чтобы увидеть детали.')
        : (isLoginAction ? 'Не удалось войти — вход не был завершён или истекло время ожидания. Попробуйте ещё раз.' : 'Проверьте лог чтобы понять причину.');

    // 1. Toast в углу — для входа/проверки показываем более развёрнутое сообщение
    // (важно, сотруднику нужно время его заметить и прочитать)
    const toastMsg = isCheckAction
      ? (isOk ? '✅ ' + actionName + ' — вы авторизованы' : '❌ ' + actionName + ' — сессия не найдена, войдите заново')
      : (isOk ? '✅ ' : '❌ ') + actionName + (isOk ? ' — успешно, сессия сохранена' : ' — не удалось, попробуйте ещё раз');
    toast(toastMsg, isOk ? 'ok' : 'err');
  ...
  function renderContent() {
    ...
    if (state.activeTab === 'queue') view.innerHTML = tmplQueue();
    bindPublishInputs();
  }
  ...
  async function runAction(action) {
    try {
      ...
      const labels = {
        ...
        'login-vk': 'Открываю окно входа ВКонтакте — введите телефон и код из SMS',
        'login-ok': 'Открываю окно входа Одноклассники — введите логин и пароль',
        'login-dzen': 'Открываю окно входа Дзен — введите логин и пароль',
        'login-max': 'Открываю окно входа Макс — введите телефон и код из SMS',
        'check-vk': 'Проверяю статус входа ВКонтакте…',
        'check-ok': 'Проверяю статус входа Одноклассники…',
        'check-dzen': 'Проверяю статус входа Дзен…',
        'check-max': 'Проверяю статус входа Макс…',
      };
    ...
    logPollTimer = setInterval(async () => {
      try {
        ...
        if (r.status === 'done' || r.status === 'error') {
          ...
          // ВАЖНО: НЕ мутируем state.server.process ЗДЕСЬ перед вызовом refreshServerState() —
          // иначе refreshServerState() увидит уже новый статус вместо предыдущего 'running'
          // и решит, что процесс "только что завершился" никогда не было true (justFinished
          // всегда false), из-за чего notifyProcessFinished() (и тост о входе/проверке
          // соцсетей) вообще никогда не срабатывал. refreshServerState сама обновит
          // state.server свежими данными с сервера.
          await refreshServerState();
          // Отчёт «✅ Готово: X/Y городов успешно» относится ТОЛЬКО к публикации
          // Яндекс.Бизнес. Раньше условие не проверяло r.action, поэтому это
          // сообщение про города всплывало и после входа/проверки в соцсетях —
          // сбивало с толку на странице Настроек, где речь вообще не о городах.
          if (wasRunning && r.action === 'publish' && state.server.reports > 0) {
            try {
          ...
          setTimeout(() => { if (state.activeTab === 'run' || state.activeTab === 'settings') renderContent(); }, 400);
        }
      } catch {}
    }, 900);
  }
  ...
  function bindPublishInputs() {
    ...
    // ── Соцсети (отдельный пост на бренд, свой текст/картинки) ──
    const st = $('#fldSocialText');
    if (st) {
      autoGrowTextarea(st);
      st.addEventListener('input', e => {
        autoGrowTextarea(e.target);
        state.socialPost.text = e.target.value;
      });
    }
    const si = $('#fldSocialImg');
    if (si) {
      autoGrowTextarea(si);
      si.addEventListener('input', e => {
        autoGrowTextarea(e.target);
        state.socialPost.imageUrl = e.target.value;
      });
    }
    const sFileInput = $('#fldSocialImgFile');
    if (sFileInput) {
      sFileInput.addEventListener('change', async (e) => {
        ...
        for (const file of files) {
          ...
          try {
            ...
            if (!Array.isArray(state.socialPost.localImages)) state.socialPost.localImages = [];
            // Ограничение: максимум 10 (наибольший лимит среди площадок — VK/Telegram)
            if (state.socialPost.localImages.length >= 10) {
              toast(`Максимум 10 фото для соцсетей — файл "${file.name}" не добавлен.`, 'err');
              try { await fetch('/api/uploads/' + encodeURIComponent(data.fileName), { method: 'DELETE' }); } catch {}
              continue;
            }
            state.socialPost.localImages.push({
              fileName: data.fileName,
              url: data.url,
              path: data.path,
              originalName: data.originalName,
  ...
  const OTHER_PLATFORMS = [
    ...
    { id: 'ok', title: '🧡 Одноклассники' },
    { id: 'dzen', title: '🟡 Дзен' },
    { id: 'max', title: '⬛ Макс' },
  ];

  // ── Авто-адаптация текста под площадку (клиентская копия text-adapt.js для live-превью). ──
  // Синтаксис: [текст ссылки](https://...). Telegram — реальный кликабельный анкор.
  // ВК (через API) — экспериментальная разметка [url|текст]. Остальное — "текст: url"
  // (анкоров платформа не поддерживает, ссылка остаётся видимым текстом).
  const SOCIAL_ANCHOR_RE = /\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g;
  const adaptTextPreview = (text, mode) => {
    const t = text || '';
    if (mode === 'telegram') {
      return t.replace(new RegExp(SOCIAL_ANCHOR_RE), (_, label, url) => '🔗' + label);
    }
    if (mode === 'vk') {
      return t.replace(new RegExp(SOCIAL_ANCHOR_RE), (_, label, url) => `[${url}|${label}]`);
    }
    return t.replace(new RegExp(SOCIAL_ANCHOR_RE), (_, label, url) => `${label}: ${url}`);
  };
  const PLATFORM_ADAPT_MODE = { telegram: 'telegram', vk: 'vk', ok: 'plain', dzen: 'plain', max: 'plain' };

  function tmplOtherPlatforms() {
    const sp = state.socialPost;

    // Список загруженных локальных файлов соцпоста (свой, независимый от Яндекс-поста)
    const localImagesHtml = (sp.localImages && sp.localImages.length > 0)
      ? '<div class="local-images-list" style="display:flex;flex-wrap:wrap;gap:8px;margin-top:10px">'
        + sp.localImages.map((img, idx) =>
            '<div class="local-image-chip" style="position:relative;background:var(--bg-2);border:1px solid var(--border);border-radius:8px;padding:6px 32px 6px 8px;display:flex;align-items:center;gap:8px">'
            + '<img src="' + esc(img.url) + '" style="width:36px;height:36px;object-fit:cover;border-radius:4px" alt="">'
            + '<div style="display:flex;flex-direction:column;gap:2px;min-width:0">'
            + '<span style="font-size:12px;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:160px" title="' + esc(img.originalName || img.fileName) + '">' + esc(img.originalName || img.fileName) + '</span>'
            + '<span style="font-size:10px;color:var(--muted)">локальный файл</span>'
            + '</div>'
            + '<button type="button" class="btn-x" onclick="window.__app.removeSocialLocalImage(' + idx + ')" style="position:absolute;right:6px;top:50%;transform:translateY(-50%);width:20px;height:20px;border-radius:50%;border:none;background:rgba(255,80,80,0.15);color:#ff6464;cursor:pointer;font-size:13px;display:flex;align-items:center;justify-content:center;padding:0" title="Удалить файл">×</button>'
            + '</div>'
          ).join('')
        + '</div>'
      : '';

    // ── Превью авто-адаптации: показываем как текст будет выглядеть на каждой
    // выбранной площадке. Анкорные ссылки [текст](url) → реальный клик в Telegram,
    // разметка [url|текст] в ВК, "текст: url" во всём остальном (см. text-adapt.js).
    const selectedForPreview = OTHER_PLATFORMS.filter(pl => sp.platforms[pl.id]);
    const hasAnchorSyntax = SOCIAL_ANCHOR_RE.test(sp.text || '');
    SOCIAL_ANCHOR_RE.lastIndex = 0; // сброс — regex с флагом g хранит состояние между вызовами .test()
    const adaptPreviewHtml = (sp.text.trim() && selectedForPreview.length > 0)
      ? '<div class="field" style="margin-top:14px">'
        + '<div class="row" style="justify-content:space-between;align-items:center">'
        + '<label class="label" style="margin:0">👁 Превью адаптации по площадкам</label>'
        + '<button type="button" class="btn btn-ghost btn-xs" onclick="window.__app.refreshSocialPreview()">🔄 Обновить превью</button>'
        + '</div>'
        + selectedForPreview.map(pl => {
            let mode = PLATFORM_ADAPT_MODE[pl.id];
            let anchorNote;
            if (pl.id === 'ok') {
              // ОК — два способа публикации. Через API анкоров нет (текст:url).
              // Через браузер редактор ОК умеет вставлять НАСТОЯЩИЕ кликабельные
              // ссылки через тулбар — покажем это в превью, если заполнены
              // именно браузерные поля (логин/пароль), а не API-ключи.
              const okCfg = state.social.ok || {};
              const hasApiCreds = okCfg.accessToken && okCfg.publicKey && okCfg.secretKey && okCfg.groupId;
              const hasBrowserCreds = okCfg.login && okCfg.password;
              if (!hasApiCreds && hasBrowserCreds) {
                anchorNote = hasAnchorSyntax ? ' · 🔗 анкор кликабелен (через браузер, тулбар «Ссылка»)' : '';
              } else {
                anchorNote = hasAnchorSyntax ? ' · ссылка видна как текст (анкоры недоступны через API)' : '';
              }
            } else {
              anchorNote = mode === 'telegram' ? ' · 🔗 анкор кликабелен' : mode === 'vk' ? ' · 🔗 экспериментальная разметка ВК' : hasAnchorSyntax ? ' · ссылка видна как текст (анкоры не поддерживаются)' : '';
            }
            const adapted = adaptTextPreview(sp.text, mode);
            return '<div style="margin-bottom:8px;padding:8px 10px;background:var(--bg-2);border:1px solid var(--border);border-radius:8px">'
              + '<div class="hint" style="margin-bottom:4px">' + pl.title + anchorNote + '</div>'
              + '<div style="font-size:13px;white-space:pre-wrap">' + esc(adapted) + '</div>'
              + '</div>';
          }).join('')
        + '</div>'
      : '';
    ...
    const textCard = '<div class="card">'
      + '<div class="card-header"><div class="card-title"><span class="card-title-ico">📡</span>Соцсети — текст поста</div></div>'
      + '<div class="hint" style="margin-bottom:10px">Отдельно от Яндекс.Бизнеса выше: публикуется ОДИН пост на бренд, без разбивки по странам/городам. Свой текст, свои картинки.</div>'
      + '<div class="field">'
      + '<label class="label">Текст поста для соцсетей</label>'
      + '<textarea class="textarea textarea-autogrow" id="fldSocialText" rows="5" placeholder="Текст поста для Telegram/ВК/ОК/Дзен/Макс...">' + esc(sp.text) + '</textarea>'
      + '<div class="hint">Чтобы сделать кликабельную ссылку — напишите <code>[текст](https://...)</code>. Полностью работает в Telegram; для остальных площадок ссылка автоматически превратится в видимый текст.</div>'
      + '</div>'
      + adaptPreviewHtml
      + '<div class="field">'
      + '<label class="label">Картинки <span style="color:var(--muted);font-weight:500;text-transform:none;letter-spacing:0">(до 10 — по площадкам лимиты разные)</span></label>'
      + '<div style="display:flex;gap:8px;align-items:flex-start;flex-wrap:wrap">'
      + '<textarea class="textarea input-mono" id="fldSocialImg" rows="3" placeholder="Можно: ссылки (по строке) ИЛИ загрузить файлы кнопкой справа &#10;https://ibb.co/abc/foto1.jpg" style="flex:1;min-width:240px">' + esc(sp.imageUrl) + '</textarea>'
      + '<div style="display:flex;flex-direction:column;gap:6px">'
      + '<input type="file" id="fldSocialImgFile" accept="image/jpeg,image/png,image/webp,image/gif" multiple style="display:none">'
      + '<button type="button" class="btn btn-secondary" onclick="document.getElementById(\'fldSocialImgFile\').click()" style="white-space:nowrap">📁 Выбрать файл</button>'
      + '<span style="font-size:11px;color:var(--muted);text-align:center">до 20 МБ</span>'
      + '</div>'
      + '</div>'
      + localImagesHtml
      + '<div class="hint">Яндекс/ОК используют только 1-е фото, ВК/Telegram — до 10.</div>'
      + '</div>'
      + '</div>';

    const checks = OTHER_PLATFORMS.map(pl => {
      const checked = sp.platforms[pl.id] ? 'checked' : '';
      return '<label class="checkbox-row" style="display:flex;align-items:center;gap:8px;margin-bottom:8px">'
        + '<input type="checkbox" ' + checked + ' onchange="window.__app.toggleSocialPlatform(\'' + pl.id + '\',this.checked)">'
        + '<span>' + pl.title + '</span>'
        + '</label>';
    }).join('');

    const dzenSub = sp.platforms.dzen
      ? '<div class="field" style="margin-left:26px;margin-top:-4px">'
        + '<label class="label">Тип публикации в Дзен</label>'
        + '<div class="row" style="gap:10px">'
        + '<label><input type="radio" name="dzenPubType" value="article" ' + (sp.dzenPubType === 'article' ? 'checked' : '') + ' onchange="window.__app.setSocialDzenPubType(\'article\')"> Статья</label>'
        + '<label><input type="radio" name="dzenPubType" value="post" ' + (sp.dzenPubType === 'post' ? 'checked' : '') + ' onchange="window.__app.setSocialDzenPubType(\'post\')"> Пост</label>'
        + '</div></div>'
      : '';

    const scheduleBlock = sp.publishMode === 'schedule'
      ? '<div class="field" style="margin-top:10px">'
        + '<label class="label">Дата и время публикации</label>'
        + '<input class="input" type="datetime-local" value="' + esc(sp.scheduledAt) + '" onchange="window.__app.setSocialScheduledAt(this.value)">'
        + '</div>'
      : '';

    const anySelected = OTHER_PLATFORMS.some(pl => sp.platforms[pl.id]);

    const controlsCard = '<div class="card">'
      + '<div class="card-header"><div class="card-title">Куда публиковать</div></div>'
      + checks
      + dzenSub
      + '<hr class="divider">'
      + '<div class="row" style="gap:14px;align-items:center">'
      + '<label><input type="radio" name="publishMode" value="now" ' + (sp.publishMode === 'now' ? 'checked' : '') + ' onchange="window.__app.setSocialPublishMode(\'now\')"> Опубликовать сейчас</label>'
      + '<label><input type="radio" name="publishMode" value="schedule" ' + (sp.publishMode === 'schedule' ? 'checked' : '') + ' onchange="window.__app.setSocialPublishMode(\'schedule\')"> Запланировать</label>'
      + '</div>'
      + scheduleBlock
      + '<div style="margin-top:14px">'
      + '<button class="btn btn-secondary btn-block" onclick="window.__app.submitOtherPlatforms()" ' + (anySelected ? '' : 'disabled') + '>'
      + (sp.publishMode === 'schedule' ? '🗓 Запланировать публикацию' : '📤 Опубликовать на выбранных площадках')
      + '</button>'
      + '</div>'
      + '</div>';

    // Явный визуальный разрыв с секцией Яндекса — чтобы не путать «пост на бренд»
    // с «пост по городам».
    return '<div class="section-label" style="margin-top:24px;padding-top:14px;border-top:2px solid var(--border)">📡 Соцсети (отдельно от Яндекс.Бизнеса — один пост на бренд)</div>'
      + textCard
      + controlsCard;
  }

  function toggleSocialPlatform(id, checked) {
    state.socialPost.platforms[id] = checked;
    renderContent();
  }
  // Текст уже сохраняется в state.socialPost.text при каждом вводе (см. bindPublishInputs),
  // но полный renderContent() на каждую букву сбросил бы фокус/курсор в textarea —
  // поэтому превью адаптации обновляется по кнопке, а не в реальном времени.
  function refreshSocialPreview() { renderContent(); }
  function setSocialDzenPubType(type) { state.socialPost.dzenPubType = type; }
  function setSocialPublishMode(mode) { state.socialPost.publishMode = mode; renderContent(); }
  function setSocialScheduledAt(value) { state.socialPost.scheduledAt = value; }
  async function removeSocialLocalImage(index) {
    if (!Array.isArray(state.socialPost.localImages)) return;
    const img = state.socialPost.localImages[index];
    if (img && img.fileName) {
      try { await fetch('/api/uploads/' + encodeURIComponent(img.fileName), { method: 'DELETE' }); } catch {}
    }
    state.socialPost.localImages.splice(index, 1);
    renderContent();
  }

  async function submitOtherPlatforms() {
    const sp = state.socialPost;
    const selected = OTHER_PLATFORMS.filter(pl => sp.platforms[pl.id]).map(pl => pl.id);
    if (selected.length === 0) { toast('Выберите хотя бы одну площадку', 'err'); return; }
    if (!sp.text.trim()) { toast('Заполните текст поста для соцсетей', 'err'); return; }
    if (sp.publishMode === 'schedule' && !sp.scheduledAt) { toast('Укажите дату и время публикации', 'err'); return; }

    const localImages = Array.isArray(sp.localImages) ? sp.localImages : [];
    const capLimits = { telegram: 10, vk: 10, ok: 1, dzen: 1, max: 1 };
    const buildImagePaths = (platforms) => {
      const minCap = Math.min(...platforms.map(id => capLimits[id] || 1));
      return localImages.map(img => img.path).slice(0, minCap);
    };

    try {
      if (sp.publishMode === 'schedule') {
        const scheduledTime = new Date(sp.scheduledAt).toISOString();
        // ВК (браузерный режим) умеет планировать НАТИВНО, внутри самого ВК — публикацию
        // в нужный момент делает сам ВК, Click не обязан быть запущен в это время.
        // Поэтому для ВК отправляем отдельный немедленный запрос со scheduledTime
        // (vk_automation.js сам откроет календарь ВК и поставит пост в очередь).
        // Остальные площадки (без нативного планирования) — как раньше, через нашу
        // собственную очередь + cron.
        const vkSelected = selected.includes('vk');
        const otherPlatforms = selected.filter(id => id !== 'vk');

        if (vkSelected) {
          await api('/api/queue/publish-now', {
            method: 'POST',
            body: { text: sp.text, imagePaths: buildImagePaths(['vk']), platforms: ['vk'], scheduledTime },
          });
        }
        if (otherPlatforms.length > 0) {
          await api('/api/queue/add', {
            method: 'POST',
            body: { text: sp.text, imagePaths: buildImagePaths(otherPlatforms), platforms: otherPlatforms, dzenPubType: sp.dzenPubType, scheduledTime },
          });
        }
        toast('Запланировано на ' + new Date(scheduledTime).toLocaleString('ru-RU'), 'ok');
      } else {
        const payload = { text: sp.text, imagePaths: buildImagePaths(selected), platforms: selected, dzenPubType: sp.dzenPubType };
        const r = await api('/api/queue/publish-now', { method: 'POST', body: payload });
        const failed = Object.entries(r.results || {}).filter(([, res]) => res && res.ok === false);
        if (failed.length > 0) {
          toast('Опубликовано с ошибками: ' + failed.map(([k]) => k).join(', '), 'err');
        } else {
          toast('Опубликовано на: ' + selected.join(', '), 'ok');
        }
      }
      state.socialPost.platforms = { telegram: false, vk: false, ok: false, dzen: false, max: false };
      state.socialPost.publishMode = 'now';
      state.socialPost.scheduledAt = '';
      renderContent();
    } catch (e) {
  ...
  const SOCIAL_PLATFORMS = [
    { id: 'telegram', title: '📨 Telegram', fields: [
      { key: 'botToken', label: 'Bot Token', placeholder: '123456:AA...' },
      { key: 'chatId', label: 'Chat ID / @username канала', placeholder: '@my_channel' },
    ] },
    { id: 'vk', title: '💙 ВКонтакте', note: 'Два способа на выбор — заполните один из них. Если заполнены оба, используется способ через API (стабильнее). Ссылки сообществ: ИМП — vk.com/inmetprom, СМУ — vk.com/stalmetural, МПЭ — vk.com/metpromenergo.', fieldGroups: [
      { title: 'Способ 1 — через API (токен группы)', fields: [
        { key: 'token', label: 'Токен группы', placeholder: 'vk1.a....' },
        { key: 'groupId', label: 'ID группы', placeholder: '123456789' },
      ] },
      { title: 'Способ 2 — запасной, через браузер (вход по SMS-коду)', note: 'У ВК нет пароля в этом способе — вход по телефону + SMS-код. Укажите телефон, затем нажмите «Войти через браузер» ниже и введите код из SMS в открывшемся окне.', loginAction: 'login-vk', fields: [
        { key: 'phone', label: 'Телефон', placeholder: '+7...' },
        { key: 'groupUrl', label: 'Ссылка на сообщество', placeholder: 'https://vk.com/inmetprom' },
      ] },
    ] },
    { id: 'ok', title: '🧡 Одноклассники', note: 'Два способа на выбор — заполните один из них. Если заполнены оба, используется способ через API (стабильнее).', fieldGroups: [
      { title: 'Способ 1 — через API приложения ОК', fields: [
        { key: 'accessToken', label: 'Access Token', placeholder: '...' },
        { key: 'publicKey', label: 'Application key (public)', placeholder: '...' },
        { key: 'secretKey', label: 'Application secret key', placeholder: '...', pw: true },
        { key: 'groupId', label: 'ID группы', placeholder: '123456789' },
      ] },
      { title: 'Способ 2 — запасной, через браузер (логин/пароль)', note: 'Укажите логин и пароль, затем нажмите «Войти через браузер» ниже — окно входа откроется прямо на сервере, терминал не нужен.', loginAction: 'login-ok', fields: [
        { key: 'login', label: 'Логин (email/телефон)', placeholder: 'email или телефон' },
        { key: 'password', label: 'Пароль', placeholder: '••••••••', pw: true },
        { key: 'groupUrl', label: 'Ссылка на группу', placeholder: 'https://ok.ru/group/1234567890' },
      ] },
    ] },
    { id: 'dzen', title: '🟡 Дзен', browserAuth: true, loginAction: 'login-dzen', note: 'Укажите логин и пароль, затем нажмите «Войти через браузер» ниже — окно входа откроется прямо на сервере, терминал не нужен.', fields: [
      { key: 'login', label: 'Логин', placeholder: 'email или телефон' },
      { key: 'password', label: 'Пароль', placeholder: '••••••••', pw: true },
      { key: 'groupUrl', label: 'Ссылка на редактор канала', placeholder: 'https://dzen.ru/profile/editor/inmetprom' },
    ] },
    { id: 'max', title: '⬛ Макс', browserAuth: true, loginAction: 'login-max', note: 'У Макса нет пароля — вход по SMS-коду. Укажите телефон, затем нажмите «Войти через браузер» ниже и введите код из SMS в открывшемся окне.', fields: [
      { key: 'phone', label: 'Телефон', placeholder: '+7...' },
    ] },
  ];

  function tmplSocialSettings() {
    // Для Telegram/VK/ОК проверка — реальный запрос к API, ✅ значит подключение
    // действительно работает. Для Дзена/Макса (браузерная автоматизация без готового
    // API) кнопка сейчас проверяет только «поля заполнены» — это НЕ подтверждает,
    // что вход реально пройдёт. Показываем честную формулировку, а не «подключено».
    const statusBadge = (id, browserAuth) => {
      const s = state.socialTestStatus[id];
      if (!s) return '';
      if (s.status === 'checking') return '<span class="hint">⏳ проверка…</span>';
      if (s.status === 'ok') {
        // Для ОК режим определяется ответом сервера (mode: 'api'|'browser'),
        // для Дзена/Макса — всегда browser (только поля проверены, не сам вход).
        const isBrowserMode = browserAuth || s.mode === 'browser';
        return isBrowserMode
          ? '<span style="color:var(--acc)">ℹ️ поля заполнены (реальный вход не проверялся)</span>'
          : '<span style="color:var(--green)">✅ подключено</span>';
      }
      if (s.status === 'err') return '<span style="color:var(--red)">❌ ошибка</span>';
      return '';
    };
    const renderField = (pl, f, cfg) =>
      '<div class="field">'
      + '<label class="label">' + esc(f.label) + '</label>'
      + '<input class="input input-mono" type="' + (f.pw ? 'password' : 'text') + '" '
      + 'value="' + esc(cfg[f.key] || '') + '" placeholder="' + esc(f.placeholder || '') + '" '
      + 'oninput="window.__app.updateSocialField(\'' + pl.id + '\',\'' + f.key + '\',this.value)">'
      + '</div>';
    // Кнопка «Войти через браузер» — открывает окно входа прямо на сервере (spawn
    // node <platform>_automation.js --login через тот же /api/run, что и вход в Яндекс).
    // Позволяет сотруднику без терминала пройти ручной вход по SMS-коду/паролю.
    // Кнопка «Проверить статус входа» — честная проверка «я СЕЙЧАС реально авторизован?»
    // (фоновый headless-запрос к сохранённой сессии), не зависит от того что произошло
    // сразу после --login и не пропадает при перезагрузке страницы — можно жать когда угодно.
    const running = state.server?.process?.status === 'running';
    const runningThis = (action) => running && state.server?.process?.action === action;
    const loginButton = (loginAction) => {
      const platform = loginAction.slice('login-'.length);
      const checkAction = 'check-' + platform;
      const busyLogin = runningThis(loginAction);
      const busyCheck = runningThis(checkAction);
      const result = state.socialLoginStatus[platform];
      let badge = '';
      if (busyLogin) badge = '<span class="hint" style="margin-left:8px">⏳ окно входа открыто — введите код/пароль в браузере</span>';
      else if (busyCheck) badge = '<span class="hint" style="margin-left:8px">⏳ проверяю сессию…</span>';
      else if (result === 'ok') badge = '<span style="color:var(--green);margin-left:8px">✅ вы авторизованы</span>';
      else if (result === 'err') badge = '<span style="color:var(--red);margin-left:8px">❌ сессия не найдена — войдите заново</span>';
      return '<div style="margin-top:8px">'
        + '<button class="btn btn-secondary btn-sm" onclick="window.__app.runAction(\'' + loginAction + '\')" '
        + (running ? 'disabled' : '') + '>'
        + (busyLogin ? '⏳ Открыт браузер входа…' : '🔑 Войти через браузер')
        + '</button> '
        + '<button class="btn btn-ghost btn-sm" onclick="window.__app.runAction(\'' + checkAction + '\')" '
        + (running ? 'disabled' : '') + '>'
        + (busyCheck ? '⏳ Проверяю…' : '🔍 Проверить статус входа')
        + '</button>' + badge
        + '</div>';
    };
    const platformCard = (pl) => {
      const cfg = state.social[pl.id] || {};
      const fieldsHtml = pl.fieldGroups
        ? pl.fieldGroups.map(g =>
            '<div class="section-label" style="margin-top:10px">' + esc(g.title) + '</div>'
            + (g.note ? '<div class="hint" style="margin-bottom:8px">' + esc(g.note) + '</div>' : '')
            + g.fields.map(f => renderField(pl, f, cfg)).join('')
            + (g.loginAction ? loginButton(g.loginAction) : '')
          ).join('')
        : pl.fields.map(f => renderField(pl, f, cfg)).join('');
      return '<div class="card">'
        + '<div class="card-header"><div class="card-title">' + pl.title + '</div></div>'
        + (pl.note ? '<div class="hint" style="margin-bottom:10px">' + esc(pl.note) + '</div>' : '')
        + fieldsHtml
        + (pl.loginAction ? loginButton(pl.loginAction) : '')
        + '<div class="row" style="margin-top:12px;align-items:center;gap:10px">'
        + '<button class="btn btn-secondary btn-sm" onclick="window.__app.testSocialConnection(\'' + pl.id + '\')">🔌 ' + (pl.browserAuth ? 'Проверить поля' : 'Проверить подключение') + '</button>'
        + statusBadge(pl.id, pl.browserAuth)
        + '</div>'
        + '</div>';
    };
    return '<div class="section-label" style="margin-top:8px">📡 Соцсети</div>'
      + SOCIAL_PLATFORMS.map(platformCard).join('')
      + '<div style="margin-top:-8px;margin-bottom:16px"><button class="btn btn-primary" onclick="window.__app.saveSocialSettings()">💾 Сохранить токены соцсетей</button></div>';
  }
  const PLATFORM_ICON = { yandex: '🟥', telegram: '📨', vk: '💙', ok: '🧡', dzen: '🟡', max: '⬛' };
  let queueItemsCache = [];
  async function refreshQueueList() {
    try {
      const r = await api('/api/queue');
      queueItemsCache = r.items || [];
      renderContent();
    } catch (e) { console.error('refreshQueueList error:', e); }
  }
  async function cancelQueueItem(id) {
    try {
      await api('/api/queue/cancel', { method: 'POST', body: { id } });
      toast('Отменено', 'ok');
      await refreshQueueList();
    } catch (e) { toast('Ошибка: ' + e.message, 'err'); }
  }
  function tmplQueue() {
    if (!activeProject()) return emptyNoProject();
    if (queueItemsCache.length === 0) {
      return '<div class="card"><div class="hint">Очередь пуста. Запланированные посты для Telegram/ВК/ОК/Дзен/Макс появятся здесь.</div></div>';
    }
    const statusLabel = { pending: '⏳ ожидает', published: '✅ опубликовано', failed: '❌ ошибка', cancelled: '🚫 отменено' };
    const items = queueItemsCache.slice().sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || ''));
    const rows = items.map(item => {
      const icons = (item.platforms || []).map(p => PLATFORM_ICON[p] || '❓').join(' ');
      const preview = (item.text || '').slice(0, 90) + ((item.text || '').length > 90 ? '…' : '');
      const dateStr = item.scheduledTime ? new Date(item.scheduledTime).toLocaleString('ru-RU') : '—';
      const rightControl = item.status === 'pending'
        ? '<button class="btn btn-ghost btn-sm" style="color:var(--red)" onclick="window.__app.cancelQueueItem(\'' + item.id + '\')">🚫 Отменить</button>'
        : '<span class="hint">' + dateStr + '</span>';
      return '<div class="queue-item">'
        + '<div class="queue-item-head">'
        + '<div class="queue-item-title"><span>' + icons + '</span><span class="badge badge-muted">' + (statusLabel[item.status] || item.status) + '</span></div>'
        + rightControl
        + '</div>'
        + '<div class="queue-item-text">' + esc(preview) + '</div>'
        + (item.status === 'pending' ? '<div class="hint">Запланировано: ' + dateStr + '</div>' : '')
        + '</div>';
    }).join('');
    return '<div class="card">'
      + '<div class="card-header"><div class="card-title"><span class="card-title-ico">📬</span>Очередь публикаций</div>'
      + '<button class="btn btn-ghost btn-sm" onclick="window.__app.refreshQueueList()">🔄 Обновить</button>'
      + '</div>'
  ...
  window.__app = {
    ...
    toggleSocialPlatform, setSocialDzenPubType, setSocialPublishMode, setSocialScheduledAt,
    submitOtherPlatforms, removeSocialLocalImage, refreshSocialPreview,
    refreshQueueList, cancelQueueItem,
    ...
    updateSocialField, saveSocialSettings, testSocialConnection,
    // Актуализация
...
<div class="shell">
  ...
  <nav class="tabs-bar">
    ...
    <button class="tab"        data-tab="queue"     onclick="window.__app.switchTab('queue')"><span class="tab-ico">📬</span><span>Очередь</span></button>
